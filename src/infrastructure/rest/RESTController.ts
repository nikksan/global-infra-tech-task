...
export default abstract class RESTController {
    ...
    protected sendData(ctx: ParameterizedContext, data: unknown) {
        ctx.body = { data, error: null };
        ctx.status = 200;
    }

    protected sendError(ctx: ParameterizedContext, error: RESTError) {
        ctx.body = { data: null, error };
        ctx.status = error.status;
    }

    protected handleError(ctx: ParameterizedContext, err: Error) {
